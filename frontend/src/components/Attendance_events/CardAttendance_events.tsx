...
const CardAttendance_events = ({
  ...
  return (
    <div className={'p-4'}>
      ...
      >
        {!loading && attendance_events.map((item, index) => (
          <li
            key={item.id}
          ...
          >
              <Link href={`/attendance_events/attendance_events-view/?id=${item.id}`} className='text-lg font-bold leading-6 line-clamp-1'>
                  {item.timestamp}
              </Link>
...
export default CardAttendance_events;
