...
const CardLeave_requests = ({
  ...
  return (
    <div className={'p-4'}>
      ...
      >
        {!loading && leave_requests.map((item, index) => (
          <li
            key={item.id}
          ...
          >
              <Link href={`/leave_requests/leave_requests-view/?id=${item.id}`} className='text-lg font-bold leading-6 line-clamp-1'>
                  {item.reason}
              </Link>
...
export default CardLeave_requests;
